...
function compileFrontPanelSvg(model, opts = {}) {
  ...
  if (strip) {
    ...
    if (span > stripW) {
      throw new Error(`terminal strip "${strip.id}" does not fit: ${owned.length} terminals at ${strip.pitch} mm pitch need ${r(span / SCALE)} mm, panel offers ${r(stripW / SCALE)} mm`);
    }
